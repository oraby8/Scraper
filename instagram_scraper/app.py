...
##print(os.getcwd())
##
##file_name=str(os.getcwd()+'\'+"friends.txt")
##print(file_name)
...
def csv(fi,name):
    ...
    comment=[]
    timestamp=[]
    #url=str(fi)+'/'+str(name)+'.json'
    url='{0}/{1}.json'.format(fi, name)
    ...
    for i in range(len(data)):
        node=data[i]
        idss=node['id']
        time_s=node['taken_at_timestamp']
        timestamp.append(time_s)
        likess=node['edge_media_preview_like']['count']
        num_commentss=node['edge_media_to_comment']['count']
        ids.append(str(idss))
        likes.append(likess)
        num_comments.append(num_commentss)
        try:
            caps=node['edge_media_to_caption']['edges'][0]['node']['text']
        except:
            caps=None
        cap.append(caps)
        com=node['comments']['data']
        coms=[]
        for x in range(len(com)):
            try:
                ...
                comments=' '
            coms.append(comments)
        comment.append(coms)
            


    df = pd.DataFrame({'id':ids,'like':likes,'num_comments':num_comments,'timestamp':timestamp,'cap':cap,'comment':comment})
    #save=str(fi)+'/'+str(name)+'.csv'
    save='{0}/{1}.csv'.format(fi, name)
    print(save)
...
class InstagramScraper(object):
    ...
    def save_json(data, dst='./'):
        ...
        global file_name
        file_name=os.path.dirname(dst)[2:]
...
def main(nname,ppassword):
    ...
    parser.add_argument('--comments', action='store_true', default=True, help='Save post comments to json file')
    ...
    parser.add_argument('--retry-forever', action='store_true', default=True,
